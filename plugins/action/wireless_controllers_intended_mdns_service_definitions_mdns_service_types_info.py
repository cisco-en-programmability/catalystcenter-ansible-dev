...
def _build_action_module():
    ...
    argument_spec.update(
        dict(
            mdnsServiceDefinitionId=dict(type="str"),
    ...
    class ActionModule(ActionBase):
        ...
        def get_object(self, params):
            new_object = dict(
                mdns_service_definition_id=params.get("mdnsServiceDefinitionId"),
        ...
        def run(self, tmp=None, task_vars=None):
            ...
            if id:
                response = catalystcenter.exec(
                    family="wireless",
                    function="get_configurations_for_a_specific_instance_of_a_mdns_service_type_feature_on_a_wireless_controller_connectivity",
            ...
            if not id:
                response = catalystcenter.exec(
                    family="wireless",
                    function="get_configurations_for_mdns_service_type_feature_on_a_wireless_controller",
                    params=self.get_object(self._task.args),
                )
                self._result.update(
                    dict(catalystcenter_response=response, dnac_response=response)
