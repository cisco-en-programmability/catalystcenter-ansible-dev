...
class ActionModule(ActionBase):
    ...
    def get_object(self, params):
        new_object = dict(
            rf_profile_id=params.get("rfProfileId"),
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if id:
            response = catalystcenter.exec(
                family="wireless",
                function="get_configurations_for_a_specific_instance_of_a_rf_dca_removed_channel_config_feature_on_a_wireless_controller_connectivity",
        ...
        if not id:
            response = catalystcenter.exec(
                family="wireless",
                function="get_configurations_for_radio_frequency_feature_on_a_wireless_controller_rf_dca_removed_channel_configs",
                params=self.get_object(self._task.args),
            )
            self._result.update(
                dict(catalystcenter_response=response, dnac_response=response)
