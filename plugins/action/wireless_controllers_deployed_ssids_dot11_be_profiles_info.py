    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if id:
            response = catalystcenter.exec(
                family="wireless",
                function="get_configurations_for_a_specific_instance_of_a_deployed_dot11_be_profile_feature_on_a_wireless_controller",
        ...
        if not id:
            response = catalystcenter.exec(
                family="wireless",
                function="get_configurations_for_dot11_be_profile_feature_on_a_wireless_controller",
                params=self.get_object(self._task.args),
            )
            self._result.update(
                dict(catalystcenter_response=response, dnac_response=response)
