...
argument_spec.update(
    dict(
        locationConfigId=dict(type="str"),
...
class ActionModule(ActionBase):
    ...
    def get_object(self, params):
        new_object = dict(
            location_config_id=params.get("locationConfigId"),
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if id:
            response = catalystcenter.exec(
                family="wireless",
                function="get_configurations_for_a_specific_instance_of_a_associated_ap_config_feature_on_a_wireless_controller_connectivity",
        ...
        if not id:
            response = catalystcenter.exec(
                family="wireless",
                function="get_configurations_for_tag_mapping_feature_on_a_wireless_controller",
                params=self.get_object(self._task.args),
            )
            self._result.update(
                dict(catalystcenter_response=response, dnac_response=response)
