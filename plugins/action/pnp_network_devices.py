...
argument_spec.update(
    dict(
        ...
        removePnpProfile=dict(type="bool"),
        id=dict(type="str"),
    )
)
...
class PnpNetworkDevices(object):
    ...
    def get_all_params(self, name=None, id=None):
        ...
        new_object_params["onboarding_state"] = self.new_object.get(
            "onboardingState"
        ) or self.new_object.get("onboarding_state")
        ...
        new_object_params["smart_account"] = self.new_object.get(
            "smartAccount"
        ) or self.new_object.get("smart_account")
        new_object_params["virtual_account"] = self.new_object.get(
            "virtualAccount"
        ) or self.new_object.get("virtual_account")
        new_object_params["image_version"] = self.new_object.get(
            "imageVersion"
        ) or self.new_object.get("image_version")
        new_object_params["contacted_status"] = self.new_object.get(
            "contactedStatus"
        ) or self.new_object.get("contacted_status")
        new_object_params["svl_claimable"] = self.new_object.get(
            "svlClaimable"
        ) or self.new_object.get("svl_claimable")
        new_object_params["svl_device"] = self.new_object.get(
            "svlDevice"
        ) or self.new_object.get("svl_device")
    ...
    def delete_by_id_params(self):
        new_object_params = {}
        new_object_params["remove_pnp_profile"] = self.new_object.get(
            "remove_pnp_profile"
        )
    ...
    def get_object_by_name(self, name):
        ...
        try:
            items = self.catalystcenter.exec(
                ...
                function="get_pnp_device_details",
    ...
    def get_object_by_id(self, id):
        ...
        try:
            items = self.catalystcenter.exec(
                family="device_onboarding_pnp",
                function="get_pnp_device_details_by_id",
                params={"id": id},
            )
            if isinstance(items, dict):
                if "response" in items:
    ...
    def delete(self):
        ...
        result = self.catalystcenter.exec(
            family="device_onboarding_pnp",
            function="delete_pnp_device_by_id",
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = PnpNetworkDevices(self._task.args, catalystcenter)
        ...
        if state == "present":
            catalystcenter.fail_json(
                "This resource does not support the 'present' state, only 'absent' is supported."
            )
        elif state == "absent":
