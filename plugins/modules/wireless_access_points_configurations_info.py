...
DOCUMENTATION = r"""
---
module: wireless_access_points_configurations_info
short_description: Information module for Wireless Access Points Configurations
description:
  - Get all Wireless Access Points Configurations. - > This API endpoint allows users to query configuration information for
    one or more access points. Users can filter the results by specifying query parameters such as the Ethernet MAC address
    of a specific access point. When no query parameters are provided, the API returns details for all access points. If multiple
    query parameters are included, the API returns only those access points that match all specified filter criteria.
...
requirements:
  ...
  - python >= 3.12
seealso:
  - name: Cisco Catalyst Center documentation for Wireless RetrieveAccessPointDetails
    description: Complete reference of the RetrieveAccessPointDetails API.
    link: https://developer.cisco.com/docs/dna-center/#!retrieve-access-point-details
notes:
  - SDK Method used are
    wireless.Wireless.retrieve_access_point_details,
  - Paths used are
    get /dna/intent/api/v1/wirelessAccessPoints/configurations,
"""

EXAMPLES = r"""
---
- name: Get all Wireless Access Points Configurations
  cisco.catalystcenter.wireless_access_points_configurations_info:
