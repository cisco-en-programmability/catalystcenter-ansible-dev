...
class TestLanAutomationPnpCompletionOrder(TestCase):
    def build_workflow(self):
        ...
        workflow.want = {
            "lan_automation": {
                "primaryDeviceManagmentIPAddress": "204.1.2.4",
                "launchAndWait": True,
                "pnpAuthorization": True,
                "discoveryDevices": [{"deviceSerialNumber": "fvh2947la9k"}],
            }
        }
        workflow.params = {
            "catalystcenter_api_task_timeout": 604800,
            "catalystcenter_task_poll_interval": 0,
        }
        workflow.result = {"changed": False}
        workflow.status = "success"
        workflow.msg = ""
        workflow.started_lan_automation = []
        workflow.completed_lan_automation = []
        workflow.log = MagicMock()
        workflow.collect_logs = MagicMock(return_value=[])
        workflow.get_task_details = MagicMock(
            return_value={"isError": False, "progress": "LAN Automation complete"}
        )
        return workflow

    def test_complete_task_authorizes_pnp_before_returning(self):
        workflow = self.build_workflow()
        workflow.authorize_devices = MagicMock(return_value=["FVH2947LA9K"])
    ...
    def test_pnp_authorization_without_serials_fails_immediately(self):
        workflow = self.build_workflow()
        workflow.want["lan_automation"]["discoveryDevices"] = []
        workflow.authorize_devices = MagicMock()

        workflow.get_lan_auto_task_status(
            {"response": {"taskId": "lan-automation-task"}}
        )

        workflow.authorize_devices.assert_not_called()
        self.assertEqual(workflow.status, "failed")
        self.assertIn("no devices were given", workflow.msg)
